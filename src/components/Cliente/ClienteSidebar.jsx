...
function ServiciosIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M20.4 14.5L16 10 4 20" /></svg>; }
...
function FormulariosIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="5" y="3" width="14" height="18" rx="2" /><path d="M9 7h6M9 11h6M9 15h4" /></svg>; }
function CitasIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="4" width="18" height="18" rx="2" /><path d="M16 2v4M8 2v4M3 10h18" /></svg>; }
...
export default function ClienteSidebar({
  ...
  const go = (key) => {
    ...
    if (key === 'servicios') navigate('/ClienteServicios');
    ...
    <aside className={styles.sidebar}>
      <div className={styles.brand}>
        ...
        <div>
          ...
          <div className={styles.brandTag}>Portal cliente</div>
        </div>
      </div>

      <div className={styles.sbSection}>Menú</div>
      <NavItem icon={<HomeIcon />} label="Inicio" active={active === 'inicio'} onClick={() => go('inicio')} />
      <NavItem icon={<ServiciosIcon />} label="Servicios" badge={serviciosCount} active={active === 'servicios'} onClick={() => go('servicios')} />
      <NavItem icon={<TramitesIcon />} label="Mis trámites" badge={tramitesCount} active={active === 'tramites'} onClick={() => go('tramites')} />
      <NavItem icon={<FormulariosIcon />} label="Formularios" active={active === 'formularios'} onClick={() => go('formularios')} />
      <NavItem icon={<CitasIcon />} label="Citas" active={active === 'citas'} onClick={() => go('citas')} />
      <NavItem icon={<PagosIcon />} label="Pagos" active={active === 'pagos'} onClick={() => go('pagos')} />

      <div className={styles.sbSection}>Cuenta</div>
      ...
      <div className={styles.sbUser}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className={styles.sbUserRole}>Cliente</div>
        </div>
      </div>
      ...
      {logoutConfirmOpen && (
        <div className={styles.scrim} onClick={(e) => { if (e.target === e.currentTarget) setLogoutConfirmOpen(false); }}>
          <div className={styles.logoutModal}>
            ...
            <div className={styles.logoutText}>Tendrás que iniciar sesión de nuevo para acceder a tu portal.</div>
